...
public class DB {
	...
	public static Connection DBConnection()
	{
		...
		try
		{
			...
			conn = DriverManager.getConnection("jdbc:mysql:///store","root", "12006011");
	...
	{
            //method to establish a connection to a MySQL database
	...
	public static void deleteProductToDB(String id)
	{
		...
		try {
			...
			int status=statement.executeUpdate("DELETE from stock WHERE ProductID = '"+id+"';");
                        // one row is effected by delete operation.
		    if(status==1)
